...
export default function Reviews() {
  ...
  return (
    <div className="space-y-5">
      {/* Aggregate hero */}
      <div className="card asphalt-grain p-6 relative overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-3">
            {PLATFORMS.map((p) => (
              <div key={p.name} className="card bg-surface-2 p-3 text-center">
                <div className="data text-[10px] text-steel uppercase">{p.name}</div>
                <div className="display text-2xl mt-1" style={{ color: p.color }}>{p.rating}</div>
                <div className="data text-[10px] text-steel-dim">{p.count} reviews</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div key={t.id} className={`card p-4 ${t.active ? "" : "opacity-50"}`}>
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => updateTestimonial(t.id, { active: !t.active })} className="grid h-7 w-7 place-items-center rounded-lg card card-hover" title={t.active ? "Hide from site" : "Show on site"}>
                  {t.active ? <Eye size={13} className="text-ok" /> : <EyeOff size={13} className="text-steel" />}
                </button>
                <button onClick={() => removeTestimonial(t.id)} className="grid h-7 w-7 place-items-center rounded-lg card card-hover" title="Delete">
...
function AddReview({ onClose, onSave }: { onClose: () => void; onSave: (t: Testimonial) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative card w-full max-w-md p-5 rise" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="display text-lg text-cream">Add a review</h3>
          <button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg card"><X size={15} /></button>
        </div>
